    ...
    namespace aftermath
    {
        namespace random
        {
            ...
            struct sampler_pareto
            {
                ...
                using type = sampler_pareto<t_result_type, t_uniform_type, t_bounds_type, t_diameter>;
                using distribution_type = probability::dist_pareto<t_result_type>;
            ...
            using default_sampler_pareto_t = sampler_pareto<t_result_type, typename t_engine_type::result_type, std::size_t, t_engine_type::max() - t_engine_type::min()>;
            
            template <typename t_result_type, typename t_uniform_type>
            struct sampler_pareto<t_result_type, t_uniform_type, std::size_t, mersenne_number<32>::value>
            {
                static constexpr std::size_t diameter = mersenne_number<32>::value;

                using type = sampler_pareto<t_result_type, t_uniform_type, std::size_t, diameter>;
                using distribution_type = probability::dist_pareto<t_result_type>;
                using result_type = typename distribution_type::result_type;
                using param_type = typename distribution_type::param_type;
                using uniform_type = t_uniform_type;
