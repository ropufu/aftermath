...
{
    template <std::floating_point t_float_type, ropufu::integer t_int_type, t_int_type t_diameter = std::numeric_limits<t_int_type>::max()>
    struct rationalize
    {
    ...
    public:
        ...
        static constexpr int_type below(int_type current) noexcept
        {
            ...
            while (step != 0)
            {
                x = type::away_down(x, mark, step); // x is now stricly below the target.
                x = type::toward_up(x, mark, step / 2); // x is now at the target.
                step /= 4;
            } // while (...)
            return type::away_down(x, mark, 1);
        } // below(...)
