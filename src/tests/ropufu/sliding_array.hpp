...
TEST_CASE_TEMPLATE("testing (randomized) sliding_array", sliding_array_type, ROPUFU_AFTERMATH_TESTS_SLIDING_ARRAY_ALL_TYPES)
{
    ...
    {
        window.displace_back(sequence[i]);
...
TEST_CASE_TEMPLATE("testing empty sliding_vector", sliding_array_type, ROPUFU_AFTERMATH_TESTS_SLIDING_ARRAY_ALL_TYPES)
{
    using value_type = typename sliding_array_type::value_type;
    ...
    a.displace_back(zero);
    REQUIRE_EQ(a[0], zero);
} // TEST_CASE_TEMPLATE(...)

TEST_CASE_TEMPLATE("testing sliding_vector displace_back", sliding_array_type, ROPUFU_AFTERMATH_TESTS_SLIDING_ARRAY_ALL_TYPES)
{
    using value_type = typename sliding_array_type::value_type;
    using sliding_vector_type = ropufu::aftermath::sliding_vector<value_type>;
...
TEST_CASE_TEMPLATE("testing sliding_vector displace_front", sliding_array_type, ROPUFU_AFTERMATH_TESTS_SLIDING_ARRAY_ALL_TYPES)
{
    using value_type = typename sliding_array_type::value_type;
    using sliding_vector_type = ropufu::aftermath::sliding_vector<value_type>;
    ...
    a.displace_front(three);
    ref = {three, zero, zero, zero, zero};
    for (std::size_t i = 0; i < ref.size(); ++i) CHECK_EQ(a[i], ref[i]);

    a.displace_front(one);
    ref = {one, three, zero, zero, zero};
    for (std::size_t i = 0; i < ref.size(); ++i) CHECK_EQ(a[i], ref[i]);

    a.displace_front(zero);
    a.displace_front(two);
    ref = {two, zero, one, three, zero};
    for (std::size_t i = 0; i < ref.size(); ++i) CHECK_EQ(a[i], ref[i]);
} // TEST_CASE_TEMPLATE(...)
