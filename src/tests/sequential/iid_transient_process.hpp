...
#ifndef ROPUFU_AFTERMATH_TESTS_SEQUENTIAL_IID_TRANSIENT_PROCESS_HPP_INCLUDED
#define ROPUFU_AFTERMATH_TESTS_SEQUENTIAL_IID_TRANSIENT_PROCESS_HPP_INCLUDED
...
#include "../../ropufu/sequential/iid_transient_process.hpp"
...
#define ROPUFU_AFTERMATH_TESTS_SEQUENTIAL_IID_TRANSIENT_PROCESS_ALL_TYPES    \
...
TEST_CASE_TEMPLATE("testing iid_transient_process json", sampler_type, ROPUFU_AFTERMATH_TESTS_SEQUENTIAL_IID_TRANSIENT_PROCESS_ALL_TYPES)
{
    using process_type = ropufu::aftermath::sequential::iid_transient_process<sampler_type, sampler_type>;
    process_type proc{};
...
#ifndef ROPUFU_NO_JSON
TEST_CASE("testing iid_transient_process json (Gaussian)")
{
...
TEST_CASE_TEMPLATE("testing iid_transient_process one-at-a-time", sampler_type, ROPUFU_AFTERMATH_TESTS_SEQUENTIAL_IID_TRANSIENT_PROCESS_ALL_TYPES)
{
    using process_type = ropufu::aftermath::sequential::iid_transient_process<sampler_type, sampler_type>;
...
TEST_CASE("testing iid_transient_process changed (Uniform)")
{
    ...
    static constexpr std::size_t last_under_change_index = 4;
    static constexpr std::size_t count = 8;

    distributiont_type d1{a, b};
    distributiont_type d2{c, d};
    process_type proc {d1, d2, first_under_change_index, last_under_change_index - first_under_change_index + 1};
    for (std::size_t i = 0; i < count; ++i)
    {
        auto x = proc.next();
        if (i >= first_under_change_index && i <= last_under_change_index)
        {
            CHECK_GE(x, c);
            CHECK_LE(x, d);
        } // if (...)
...
TEST_CASE("testing iid_transient_process bulk (Gaussian)")
{
    using sampler_type = ropufu::aftermath::random::normal_sampler_512<std::mt19937>;
    using distributiont_type = typename sampler_type::distribution_type;
    using process_type = ropufu::aftermath::sequential::iid_transient_process<sampler_type, sampler_type>;
...
#endif // ROPUFU_AFTERMATH_TESTS_SEQUENTIAL_IID_TRANSIENT_PROCESS_HPP_INCLUDED
